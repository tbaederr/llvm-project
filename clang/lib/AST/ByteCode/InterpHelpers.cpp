...
void ensureArraySize(Program &P, const Pointer &Ptr,
                     unsigned RequestedIndex) {
  if (!Ptr.isBlockPointer())
    return;
 // llvm::errs() << __PRETTY_FUNCTION__ << ". Requested: " << RequestedIndex
              // << '\n';
 ...
 // llvm::errs() << "New size: " << NewArraySize.Size << " / "
              // << NewArraySize.Capacity << '\n';

  // llvm::errs() << "Ptr before realloc: " << Ptr << '\n';
