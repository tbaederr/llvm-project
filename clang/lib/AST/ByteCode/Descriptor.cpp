...
CtorFn(getCtorPrim(Type)), DtorFn(getDtorPrim(Type)),
    ElemSize(primSize(Type)), Size(ElemSize),
...
Descriptor::Descriptor(DeclOrExpr D, const Type *SourceTy,
    ...
    : Source(D), SourceType(SourceTy),
ElemDescOrRecord(Elem), 
 CtorFn(ctorArrayDesc),
      DtorFn(Elem->DtorFn ? dtorArrayDesc : nullptr) ,
      ElemSize(Elem->getAllocSize() + sizeof(InlineDescriptor)),
      Size(ElemSize * NumElems),
      AllocSize(std::max<size_t>(alignof(void *), Size)),
      IsConst(IsConst), IsMutable(IsMutable),
      IsTemporary(IsTemporary), IsArray(true){
  assert(Source && "Missing source");
}
...
Descriptor::Descriptor(DeclOrExpr D, const Descriptor *Elem, bool IsTemporary,
                       UnknownSize)
    : Source(D),  ElemDescOrRecord(Elem),
CtorFn(ctorArrayDesc), DtorFn(Elem->DtorFn ? dtorArrayDesc : nullptr),ElemSize(Elem->getAllocSize() + sizeof(InlineDescriptor)),
      Size(UnknownSizeMark), AllocSize(alignof(void *)),       IsConst(true), IsMutable(false), IsTemporary(IsTemporary), IsArray(true)
       {
  assert(Source && "Missing source");
}
...
Descriptor::Descriptor(DeclOrExpr D, const Record *R, bool IsConst,
    ...
    : Source(D), ElemDescOrRecord(R), CtorFn(ctorRecord), DtorFn(needsRecordDtor(R) ? dtorRecord : nullptr), ElemSize(std::max<size_t>(alignof(void *), R->getFullSize())),
      Size(ElemSize), AllocSize(Size), IsConst(IsConst),
      IsMutable(IsMutable), IsTemporary(IsTemporary), IsVolatile(IsVolatile)
       {
  assert(Source && "Missing source");
}
...
Descriptor::Descriptor(DeclOrExpr D)
    : Source(D), ElemSize(1), Size(1), AllocSize(0),
