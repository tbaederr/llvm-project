...
/// FIXME: This is wrong in the new interpreter
_Static_assert((long)&((struct y*)0)->y > 0, ""); // expected-error {{not an integral constant expression}} \
                                                  // expected-note {{cannot access field of null pointer}}
EVAL_EXPR(7, (int)&((struct y*)0)->y) // expected-error {{not an integer constant expression}} \
                                      // expected-note {{cannot access field of null pointer}}
...
double d = (d = 0.0); // ref-error {{not a compile-time constant}}
double d2 = ++d; // ref-error {{not a compile-time constant}}
#endif
...
char c = ((union u)(123456)).b[0]; // ref-error {{not a compile-time constant}}
...
EVAL_EXPR(50, &Test50 < (struct Test50S*)((unsigned long)&Test50 + 10)) // both-error {{not an integer constant expression}} \
                                                                        // expected-note {{comparison between '&Test50' and '&1' has unspecified value}}
...
int PR35214_x;
#ifndef NEW_INTERP
int PR35214_y = ((struct PR35214_X *)&PR35214_x)->arr[1]; // ref-error {{not a compile-time constant}}
